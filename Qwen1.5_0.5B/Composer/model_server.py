...
class ModelServer(workflow_server.WorkflowServer):
    ...
    def preprocess(self, decoded_input, run_total):
        ...
        dataset_pre = [decoded_input[i:i+bs] for i in range(0, run_total, bs)]
        dataset = self.platform_preprocess(dataset_pre)
        return dataset
    ...
    def postprocess(self, exp_output, run_total):
        ...
        exp_output_post = self.platform_postprocess(exp_output)
        outputs = list(exp_output_post.tolist())
