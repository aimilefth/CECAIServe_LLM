...
def main():
    ...
    ap.add_argument('-p', '--ask_power', type=str, default='False', help='Whether to ask for power instead of inference. Default is False')
    ap.add_argument('-s', '--shutdown', type=str, default='False', help='Whether to request server shutdown. Default is False')
    ap.add_argument('-l', '--ask_logs', type=str, default='False', help='Whether to request server logs. Default is False')
    args = ap.parse_args()
    ...
    logging.info('--ask_power         : {}'.format(args.ask_power))
    logging.info('--shutdown          : {}'.format(args.shutdown))
    logging.info('--ask_logs          : {}'.format(args.ask_logs))

    client = my_client.MyClient(args.address)
    if strtobool(args.ask_logs):
        client.ask_logs()
    elif strtobool(args.shutdown):
        client.ask_shutdown()
    elif strtobool(args.ask_metrics):
        client.ask_metrics(args.number_of_metrics)
    elif strtobool(args.ask_power):
        client.ask_power()
    else:
