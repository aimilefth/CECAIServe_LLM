...
class BaseClient:
    ...
    def send_request(self, url, dataset_path):
        ...
        raise NotImplementedError('Forgot to overload send_request. Must be overridden by my_client.py (MyClient).')
    ...
    def manage_response(self, response):
        ...
        raise NotImplementedError('Forgot to overload manage_response. Must be overridden by my_client.py (MyClient).')
    ...
    def ask_power(self):
        """Sends a request for power and gets the power metrics."""
        url = self.address + '/api/power'
        content_type = 'application/json'
        headers = {'content-type': content_type}
        # Decide the payload based on number_of_metrics
        json_input = {'data': 'placeholder'}
        start = time.perf_counter()
        # Sending the HTTP POST request with the prepared payload
        response = requests.post(url, json=json_input, headers=headers)
        end = time.perf_counter()
        logging.info('Get Power Latency: {:.2f} s'.format(end-start))
        metrics_json = response.json()
        # Writing the JSON data to an output file
        with open(os.environ['POWER_METRICS_OUTPUT'], "w") as outfile:
            outfile.write(json.dumps(metrics_json, indent=4))

    def ask_logs(self):
        """Sends a request to the server to retrieve logs."""
        url = self.address + "/api/logs"
        start = time.perf_counter()
        # Sending the HTTP POST request
        response = requests.post(url)
        end = time.perf_counter()
        logging.info("Get Logs Latency: {:.2f} ms".format((end - start) * 1000))
        if response.status_code == 200:
            logging.info("Logs retrieved successfully.")
            with open(os.environ["SERVER_LOGS_OUTPUT"], "w") as outfile:
                outfile.write(response.text)
        else:
            logging.error("Failed to retrieve logs. Status code: {}".format(response.status_code))
            print("Failed to retrieve logs.")

    def ask_shutdown(self):
        """Sends a request to the server to shut down."""
        url = self.address + '/api/shutdown'
        content_type = 'application/json'
        headers = {'content-type': content_type}
        start = time.perf_counter()
        # Sending the HTTP POST request with an empty JSON payload
        response = requests.post(url, json={}, headers=headers)
        end = time.perf_counter()
        logging.info('Shutdown request latency: {:.2f} ms'.format((end - start) * 1000))
        if response.status_code == 200:
            logging.info('Server shutdown successfully.')
            print(response.text)
        else:
            logging.error('Failed to shut down the server. Status code: {}'.format(response.status_code))
