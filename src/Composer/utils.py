...
def create_metric_dictionary(inference_metrics_dict):
    """
    Used in the metrics service.
    Currently only adds timestamp
    """
    my_dict = inference_metrics_dict.copy()
...
def filter_jsonable_dict(d):
    """
    Returns a new dictionary containing only the key-value pairs
    from the input dictionary where the value is JSON-serializable.
    """
...
def get_local_ip_addresses():
    ...
    # Method 2: Use a UDP socket to an external host
    try:
