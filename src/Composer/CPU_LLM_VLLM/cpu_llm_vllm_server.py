...
This module defines the CpuLlmVllmServer class, a specialized server implementation for CPUs using the vLLM inference engine. It inherits from ModelServer and manages the details of running high-throughput LLM inference on CPU hardware.

Overview:
- The CpuLlmVllmServer class utilizes the vLLM engine for batched, high-throughput text generation on CPUs.
- It is designed to load and execute standard vLLM-compatible Hugging Face models.
- It implements abstract methods for model lifecycle management, inference execution, and CPU-specific power measurement.

Classes:
- CpuLlmVllmServer: A platform-specific server that manages a `vllm.LLM` engine on a CPU device.

Methods:
- __init__(self, logger): Initializes the server, prepares the vLLM engine, and instantiates the power scraper.
- init_inference_engine(self): Configures and creates the `vllm.LLM` engine and `SamplingParams` for CPU execution.
...
- inference_batch(self, dataset, run_total): Runs inference for a dataset of multiple prompts by leveraging vLLM's internal batching.
...
- scrape_power_data(self): Interfaces with `power_scraper` to collect real-time power metrics from the CPU hardware.
...
class CpuLlmVllmServer(model_server.ModelServer):
