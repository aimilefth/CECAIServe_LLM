...
echo "Composer script started on: $(date -u +"%Y-%m-%d %H:%M:%S")"
...
files=(
  ...
  "${SRC_COMPOSER_DIR}/${NAME}/power_scraper.py"
...
if ! valid_hf_repo "${MODEL_NAME_ARG}"; then
  echo "MODEL_NAME_ARG='${MODEL_NAME_ARG}' is not a valid Hugging Face repo directory in $(pwd)"
  echo "Expected: directory contains >=1 *.safetensors and files: config.json, tokenizer.json, vocab.json"
  exit 1
fi
...
cp -r "${SRC_COMPOSER_DIR}"/flask_server.py "${SRC_COMPOSER_DIR}"/utils.py "${SRC_COMPOSER_DIR}"/base_server.py "${SRC_COMPOSER_DIR}"/logconfig.ini "${SRC_COMPOSER_DIR}"/${NAME}/${NAME,,}_server.py "${SRC_COMPOSER_DIR}"/${NAME}/my_server.py "${SRC_COMPOSER_DIR}"/${NAME}/power_scraper.py ../experiment_server.py ../.env  ../extra_files_dir .

docker build -f ${SRC_COMPOSER_DIR}/${NAME}/Dockerfile.${NAME,,} $build_args --tag ${REPO}:${LABEL}_${NAME,,} --push .
status=$?
if [ $status -eq 0 ]; then
...
# Remove files
rm -r flask_server.py utils.py base_server.py logconfig.ini ${NAME,,}_server.py my_server.py experiment_server.py power_scraper.py .env extra_files_dir
