...
This module defines the GpuLlmLlamaServer class, a specialized server for running GGUF models on NVIDIA GPUs using the llama-cpp-python library with CUDA support. It inherits from ModelServer and handles GPU-accelerated inference.

Overview:
- The GpuLlmLlamaServer utilizes `llama-cpp` with full GPU offloading for fast inference of LLMs.
- It is designed to load and execute models in the GGUF format on NVIDIA GPUs.
- It implements abstract methods to manage the model's lifecycle, execute inference, and collect GPU-specific power metrics.

Classes:
- GpuLlmLlamaServer: A server that manages a `llama_cpp.Llama` inference engine with GPU offloading.

Methods:
- __init__(self, logger): Initializes the server, prepares the llama.cpp engine for GPU offloading, and instantiates the power scraper.
- init_inference_engine(self): Configures and creates the `llama_cpp.Llama` inference object, setting `n_gpu_layers` to fully utilize the GPU.
...
class GpuLlmLlamaServer(model_server.ModelServer):
