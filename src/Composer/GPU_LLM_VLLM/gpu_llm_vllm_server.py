...
This module defines the GpuLlmVllmServer class, a specialized server for NVIDIA GPUs using the vLLM inference engine. It inherits from ModelServer and is optimized for high-throughput LLM serving.

Overview:
- The GpuLlmVllmServer class utilizes the vLLM engine for batched, high-throughput text generation on NVIDIA GPUs.
...
Classes:
- GpuLlmVllmServer: A platform-specific server that manages a `vllm.LLM` engine on an NVIDIA GPU.

Methods:
- __init__(self, logger): Initializes the server, prepares the vLLM engine for GPU execution, and instantiates the power scraper.
- init_inference_engine(self): Configures and creates the `vllm.LLM` engine and `SamplingParams` for GPU execution.
...
- inference_batch(self, dataset, run_total): Runs inference for a dataset of multiple prompts, leveraging vLLM's internal batching capabilities on the GPU.
...
class GpuLlmVllmServer(model_server.ModelServer):
