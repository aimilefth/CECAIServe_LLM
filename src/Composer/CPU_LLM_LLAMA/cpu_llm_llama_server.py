...
This module defines the CpuLlmLlamaServer class, a specialized server for running GGUF models on CPUs using the llama-cpp-python library. It inherits from ModelServer and handles the specifics of CPU-based inference with this framework.

Overview:
- The CpuLlmLlamaServer utilizes `llama-cpp` for efficient, CPU-bound inference of quantized language models.
- It is designed to load and execute models in the GGUF format.
- It implements abstract methods to manage the model's lifecycle, execute inference, and collect CPU-specific power metrics.

Classes:
- CpuLlmLlamaServer: A server that manages a `llama_cpp.Llama` inference engine on a CPU.

Methods:
- __init__(self, logger): Initializes the server, prepares the llama.cpp engine, and instantiates the power scraper.
- init_inference_engine(self): Configures and creates the `llama_cpp.Llama` inference object from a GGUF model file.
...
- scrape_power_data(self): Interfaces with `power_scraper` to collect real-time power metrics from the CPU.
...
class CpuLlmLlamaServer(model_server.ModelServer):
