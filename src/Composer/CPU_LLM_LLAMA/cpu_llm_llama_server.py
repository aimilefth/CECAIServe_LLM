"""
CPU + llama-cpp-python server (text-generation) for LLMs.
Follows the TF2AIF server shape: init_kernel, warm_up, experiment_single/multiple, platform_pre/postprocess.
"""
...
class CpuLlmLlamaServer(experiment_server.BaseExperimentServer):
    ...
    def _resolve_model_path(self):
        # MODEL_PATH in BaseServer is env MODEL_NAME
        # Dockerfile copies model into WORKDIR; relative name is fine
        return self.server_configs['MODEL_PATH']
    ...
    def experiment_single(self, input, run_total=1):
        ...
        text = self._generate_one(input)
        return np.array([text], dtype=object)

    def experiment_multiple(self, dataset, run_total):
        # dataset is a list of batched lists of strings (created in BaseExperimentServer)
        output_list = []
        for batch_prompts in dataset:
            gen_texts = [ self._generate_one(p) for p in batch_prompts ]
